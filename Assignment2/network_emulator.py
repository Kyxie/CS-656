...
def processPacket(packet, fromSender):
    ...
    if not isinstance(packet, bytes):
        raise RuntimeError(
            "processPacket can only process a packet encoded as bytes")
    recvd_packet = Packet(packet)
    typ, seqnum, length, data = recvd_packet.decode()
    if verbose:
        print("Packet being processed: Type={}, seqnum={}, length={}, data={}".format(
            typ, seqnum, length, data))
    if typ == 2:  # if type == EOT
        if fromSender:
            ...
            if verbose:
                print("Sending packet: Type={}, seqnum={}, length={}, data={}".format(
                    typ, seqnum, length, data))
        ...
        if not randomTrue(prob_discard):
            ...
            if fromSender:
                if typ == 0:
                    ...
                    pass
                if verbose:
                    print("Adding packet to data buffer: Type={}, seqnum={}, length={}, data={}".format(
                        typ, seqnum, length, data))
                ...
                if verbose:
                    print("Adding packet to ack buffer: Type={}, seqnum={}, length={}, data={}".format(
                        typ, seqnum, length, data))
                ack_buff.put(packet)
            ...
            if verbose:
                print("Dropped packet: Type={}, seqnum={}, length={}, data={}".format(
                    typ, seqnum, length, data))
...
def forwardFlow():
    ...
    while True:
        ...
        if verbose:
            print("Received a packet from from sender")
        new_thread = threading.Thread(
            target=processPacket, args=(packet, True,))
        new_thread.start()
...
def backwardFlow():
    ...
    while True:
        ...
        new_thread = threading.Thread(
            target=processPacket, args=(packet, False,))
        new_thread.start()
...
def delayThread(delay):
    ...
    if verbose:
        print("Packet delayed by {} milliseconds".format(delay))
    time.sleep(s_delay)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "<Maximum Delay>", help="maximum delay of the link in units of millisecond")
    parser.add_argument("<drop probability>",
                        help="packet discard probability")
    ...
    if prob_discard < 0 or prob_discard > 1:
        raise RuntimeError(
            "Probability of discarding a packet should be between 0 and 1")
    ...
    if verbose:
        print("Starting network emulator, and waiting to receive something...")
    forwardThread.start()
