r'''
Date: 2022-03-01 16:00:30
LastEditors: Kunyang Xie
LastEditTime: 2022-03-01 16:32:23
    ...
    def __init__(self, *args):
        if len(args) == 1:
            if not isinstance(args[0], bytes):
                raise RuntimeError(
                    "Received one argument and expect bytes. Got={}\n".format(type(args[0])))
            self.typ, self.seqnum, self.length, self.data = struct.unpack(
                '!iii{}s'.format(len(args[0]) - 12), args[0])
            _, _, _, self.data = struct.unpack(
                '!iii{}s'.format(self.length), args[0])
            self.data = self.data.decode('ASCII')[0:self.length]
        else:
            if len(args[3]) > 500:
                raise RuntimeError(
                    "messages to be sent should be at most 500 characters long")
            self.typ = int(args[0])
